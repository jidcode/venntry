        ...
        <ul>
          {SidebarRoutes.map((link) => (
            <li
              key={link.title}
              className={cn(
                "font-medium text-secondary/90 rounded-sm w-full hover:bg-muted",
                pathname === link.url && "text-accent bg-accent/10"
              )}
            >
              {isOpen ? (
                <Link href={link.url}>
                  <button className=" flex items-center py-2 my-2 gap-1.5 text-base font-medium p-2 tracking-wide transition-all duration-500">
                    <link.icon size={20} />
